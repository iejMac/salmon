...
def grad_step(x: torch.Tensor, model: nn.Linear, optimizer: optim.AdamW):
    y = model(x)
    loss = y.log().sum()  # arbitrary func
    loss.backward()
    optimizer.step()
    return y
...
if __name__ == "__main__":
    ...
    out_slmn = queue.get()
    print('ddp slmn: ', out_l)

    for k in ['y', 'g_w', 'g_b']:
        assert torch.allclose(out_l[k], out_pt[k]), (out_l[k] - out_pt[k]).abs().mean()
        assert torch.allclose(out_l[k], out_slmn[k]), (out_l[k] - out_slmn[k]).abs().mean()
